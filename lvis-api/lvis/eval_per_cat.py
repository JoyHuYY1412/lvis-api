...
class LVISEvalPerCat:
    def __init__(self, lvis_gt, lvis_dt, iou_type="segm", save_file_path='./'):
...
#         iouStr = '{:0.2f}:{:0.2f}'.format(self.params.iou_thrs[0], self.params.iou_thrs[-1]) \
...
#         ### 
        ...
        if len(s[s > -1]) == 0:
            ...
            dict_per_cat_result= {}
            if (iou_thr==None and area_rng=="all" and freq_group_idx==None):
#                 num_classes = len(self.lvis_gt.get_cat_ids())
                avg_ap = 0.0
                valid_classes = 0
                if summary_type == 'ap':
#                     for i in range(0, num_classes):
                    for i, cls_i in enumerate(sorted(self.lvis_gt.get_cat_ids())):
                        print('category : {0} : {1}'.format(cls_i,np.mean(s[:,:,i,:])))
                        dict_per_cat_result[cls_i] = np.mean(s[:,:,i,:])
                        if np.mean(s[:,:,i,:])!=-1:
                            avg_ap += np.mean(s[:,:,i,:])
                            valid_classes += 1
                    print('(all categories) mAP : {}'.format(avg_ap / valid_classes))
                    print(valid_classes)
                    import json; json.dump(dict_per_cat_result, open(os.path.join(self.save_file_path,'lvis_result_percat_mAP.json'),'w'))
