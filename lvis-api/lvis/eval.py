...
#         self.params.cat_ids = [126]
    ...
    def _prepare(self):
        ...
        self.img_nel = {d["id"]: d["not_exhaustive_category_ids"]
                        for d in img_data}
        ...
        self.step_groups = self._prepare_step_group()

    def _prepare_step_group(self):
        step_groups = [[] for _ in self.params.step_state_lbl]
        cat_data = self.lvis_gt.load_cats(self.params.cat_ids)
        for idx, _cat_data in enumerate(cat_data):
            step_state = _cat_data["step_state"]
            step_groups[self.params.step_state_lbl.index(
                step_state)].append(idx)
        return step_groups
    ...
    def accumulate(self):
        ...
        for cat_idx in range(num_cats):
            ...
            for area_idx in range(num_area_rngs):
                ...
                dt_m = np.concatenate([e["dt_matches"]
                                       for e in E], axis=1)[:, dt_idx]
                dt_ig = np.concatenate([e["dt_ignore"]
                                        for e in E], axis=1)[:, dt_idx]
    ...
    def _summarize(
        self, summary_type, iou_thr=None, area_rng="all", freq_group_idx=None, step_group_idx=None
    ):
        aidx = [
            idx
        ...
        if summary_type == 'ap':
            ...
            elif step_group_idx is not None:
                s = s[:, :, self.step_groups[step_group_idx], aidx]
            else:
                s = s[:, :, :, aidx]
        else:
            s = self.eval["recall"]
    ...
    def summarize(self):
        ...
        self.results["APb0"] = self._summarize('ap', step_group_idx=0)
        self.results["APt0"] = self._summarize('ap', step_group_idx=1)
        self.results["APt1"] = self._summarize('ap', step_group_idx=2)
        self.results["APt2"] = self._summarize('ap', step_group_idx=3)
        self.results["APt3"] = self._summarize('ap', step_group_idx=4)
        self.results["APt4"] = self._summarize('ap', step_group_idx=5)
        self.results["APt5"] = self._summarize('ap', step_group_idx=6)
        self.results["APt6"] = self._summarize('ap', step_group_idx=7)
        self.results["APt7"] = self._summarize('ap', step_group_idx=8)
        self.results["APt8"] = self._summarize('ap', step_group_idx=9)
        self.results["APt9"] = self._summarize('ap', step_group_idx=10)
#         self.results["APf"]  = self._summarize('ap', freq_group_idx=2)
    ...
    def print_results(self):
        ...
        for key, value in self.results.items():
            ...
            elif len(key) > 2 and key[2] in ["b", "t"]:
                cat_group_name = key[2:4]
            else:
                cat_group_name = "all"
            ...
            print(template.format(title, _type, iou,
                                  area_rng, max_dets, cat_group_name, value))
...
class Params:
    def __init__(self, iou_type):
        ...
        self.step_state_lbl = ["b0", "t0", "t1", "t2", "t3",
                               "t4", "t5", "t6", "t7", "t8", "t9"]
